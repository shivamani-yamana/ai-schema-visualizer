      ...
      <div className="flex items-center gap-2 bg-[#262626] p-2 border-1 border-gray-700 rounded-2xl w-full max-w-3xl">
        {mode === "sql" && !dbSchemaInput && (
          <Alert
            hideIconWrapper
            color="default"
            title={`Your exisiting Database schema is required. Click on + to add it`}
            className="top-0 absolute bg-transparent p-0 w-fit"
          />
        )}
        {mode === "sql" && <DBSchemaInput />}
        <Textarea
        ...
        <Button
          ...
          isDisabled={
            !userInput.trim() || isLoading || (mode === "sql" && !dbSchemaInput)
          }
